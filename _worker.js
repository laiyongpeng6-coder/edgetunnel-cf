...
async function mu_adminAPI(path, request, env) {
  ...
  if ((path === 'mu/admin' || path === 'mu/admin/') && request.method === 'GET') {
    ...
    h.push('<title>多用户管理</title>');
    h.push('<style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:-apple-system,sans-serif;background:#0f172a;color:#e2e8f0;min-height:100vh}.hd{background:linear-gradient(135deg,#1e3a5f,#0f172a);padding:20px 30px;border-bottom:1px solid #334155;display:flex;justify-content:space-between;align-items:center}.hd h1{font-size:22px;color:#38bdf8}.hd a{color:#94a3b8;text-decoration:none;font-size:14px}.ct{max-width:1200px;margin:0 auto;padding:20px}.sts{display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:16px;margin-bottom:24px}.sc{background:#1e293b;border-radius:12px;padding:20px;border:1px solid #334155}.sc .lb{color:#94a3b8;font-size:13px;margin-bottom:4px}.sc .vl{font-size:28px;font-weight:700;color:#38bdf8}.tb{display:flex;gap:12px;margin-bottom:20px;flex-wrap:wrap;align-items:center}.tb input{background:#1e293b;border:1px solid #334155;border-radius:8px;padding:10px 14px;color:#e2e8f0;font-size:14px;flex:1;min-width:200px}.btn{padding:10px 20px;border:none;border-radius:8px;font-size:14px;font-weight:600;cursor:pointer}.bp{background:#3b82f6;color:#fff}.bd{background:#ef4444;color:#fff}.bs{background:#22c55e;color:#fff}.bsm{padding:6px 12px;font-size:12px}table{width:100%;border-collapse:collapse;background:#1e293b;border-radius:12px;overflow:hidden;border:1px solid #334155}th{background:#0f172a;color:#94a3b8;font-size:12px;font-weight:600;padding:14px 16px;text-align:left}td{padding:12px 16px;border-top:1px solid #1e293b;font-size:14px}tr:hover{background:#1e3a5f20}.ba{display:inline-block;padding:3px 10px;border-radius:20px;font-size:11px;font-weight:600}.ba-ok{background:#22c55e20;color:#4ade80}.ba-off{background:#ef444420;color:#f87171}.tb2{background:#334155;border-radius:4px;height:6px;overflow:hidden;width:120px;display:inline-block;vertical-align:middle;margin-left:8px}.tf2{height:100%;border-radius:4px;background:linear-gradient(90deg,#3b82f6,#38bdf8)}.tc{font-family:monospace;font-size:12px;color:#64748b;max-width:140px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.uc{font-family:monospace;font-size:12px;color:#94a3b8}.ac{display:flex;gap:6px}.mo{display:none;position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,.7);z-index:1000;justify-content:center;align-items:center}.mo.on{display:flex}.mc{background:#1e293b;border-radius:16px;padding:30px;width:90%;max-width:500px;border:1px solid #334155}.mc h2{font-size:18px;margin-bottom:20px;color:#38bdf8}.fg{margin-bottom:16px}.fg label{display:block;font-size:13px;color:#94a3b8;margin-bottom:6px}.fg input{width:100%;background:#0f172a;border:1px solid #334155;border-radius:8px;padding:10px 14px;color:#e2e8f0;font-size:14px}.ma{display:flex;gap:10px;justify-content:flex-end;margin-top:20px}.tt{position:fixed;top:20px;right:20px;background:#22c55e;color:#fff;padding:12px 20px;border-radius:8px;font-size:14px;z-index:2000;display:none}.tt.sh{display:block}.tt.er{background:#ef4444}.em{text-align:center;padding:60px;color:#64748b}.uc,.tc{word-break:break-all;white-space:normal}</style></head><body>');
    h.push('<div class="hd"><h1>多用户订阅管理</h1><a href="/admin">返回管理员面板</a></div>');
    h.push('<div class="ct"><div class="sts"><div class="sc"><div class="lb">总用户数</div><div class="vl" id="s1">-</div></div><div class="sc"><div class="lb">活跃用户</div><div class="vl" id="s2">-</div></div><div class="sc"><div class="lb">本月已用流量</div><div class="vl" id="s3">-</div></div><div class="sc"><div class="lb">总分配流量</div><div class="vl" id="s4">-</div></div></div>');
    h.push('<div class="tb"><input type="text" id="si" placeholder="搜索用户名..."><button class="btn bp" id="btnAdd">+ 创建用户</button><button class="btn bs" id="btnRefresh">刷新</button></div>');
    h.push('<table><thead><tr><th>用户名</th><th>UUID</th><th>TOKEN</th><th>流量</th><th>到期时间</th><th>状态</th><th>操作</th></tr></thead><tbody id="ut"><tr><td colspan="7" class="em">加载中...</td></tr></tbody></table></div>');
    h.push('<div class="mo" id="modal"><div class="mc"><h2 id="mt">创建用户</h2><input type="hidden" id="eid"><div class="fg"><label>用户名</label><input type="text" id="in1" placeholder="输入用户名"></div><div class="fg"><label>月流量限制 (GB, 0=无限)</label><input type="number" id="in2" value="0" min="0"></div><div class="fg"><label>到期天数 (0=永不过期)</label><input type="number" id="in3" value="0" min="0"></div><div class="ma"><button class="btn" style="background:#334155;color:#e2e8f0" id="btnCancel">取消</button><button class="btn bp" id="btnSubmit">创建</button></div></div></div>');
    h.push('<div class="mo" id="dm"><div class="mc" style="max-width:600px"><h2 id="dt">用户详情</h2><div id="dc"></div><div class="ma"><button class="btn" style="background:#334155;color:#e2e8f0" id="btnClose">关闭</button></div></div></div>');
    h.push('<div class="tt" id="toast"></div>');
    h.push('<script>');
    h.push("var AU=[];");
    h.push("function toast(m,e){var t=document.getElementById('toast');t.textContent=m;t.className='tt sh'+(e?' er':'');setTimeout(function(){t.className='tt'},3000)}");
    h.push("function fb(b){if(!b)return'0 B';var u=['B','KB','MB','GB','TB'];var i=Math.floor(Math.log(b)/Math.log(1024));return(b/Math.pow(1024,i)).toFixed(1)+' '+u[i]}");
    h.push("function fd(d){if(!d)return'永不过期';return new Date(d).toLocaleDateString('zh-CN')}");
    h.push("function renderTable(users){var t=document.getElementById('ut');if(!users||!users.length){t.innerHTML='<tr><td colspan="7" class="em">暂无用户</td></tr>';return}var s='';users.forEach(function(u){var used=u.used_bytes||0;var total=u.monthly_bytes||0;var pct=total>0?Math.min(100,used/total*100).toFixed(0):0;var badge=u.enabled!==false?'<span class="ba ba-ok">正常</span>':'<span class="ba ba-off">禁用</span>';var tf=total>0?fb(used)+'/'+fb(total)+'<div class="tb2"><div class="tf2" style="width:'+pct+'%"></div></div>':fb(used)+' (无限)';s+='<tr><td><strong>'+u.name+'</strong></td><td class="uc">'+(u.uuid||'').substring(0,8)+'...</td><td class="tc">'+u.token+'</td><td>'+tf+'</td><td>'+fd(u.expires_at)+'</td><td>'+badge+'</td><td class="ac"><button class="btn bsm bp" data-act="detail" data-id="'+u.id+'">详情</button> <button class="btn bsm bd" data-act="delete" data-id="'+u.id+'" data-name="'+u.name+'">删除</button></td></tr>'});t.innerHTML=s}");
    h.push("function updateStats(u){document.getElementById('s1").textContent=u.length;document.getElementById('s2").textContent=u.filter(function(x){return x.enabled!==false}).length;var ut=0;u.forEach(function(x){ut+=x.used_bytes||0});document.getElementById('s3").textContent=fb(ut);var tt=0;u.forEach(function(x){tt+=x.monthly_bytes||0});document.getElementById('s4").textContent=tt>0?fb(tt):'无限'}");
    h.push("async function loadUsers(){try{var r=await fetch('/mu/admin/allusers');var d=await r.json();if(d.success){AU=d.users||[];renderTable(AU);updateStats(AU)}else{toast('加载失败',true)}}catch(e){toast('网络错误',true)}}");
    h.push("document.getElementById('btnAdd').onclick=function(){document.getElementById('mt').textContent='创建用户';document.getElementById('eid').value='';document.getElementById('in1').value='';document.getElementById('in2').value='0';document.getElementById('in3').value='0';document.getElementById('btnSubmit').textContent='创建';document.getElementById('modal').classList.add('on')};");
    h.push("document.getElementById('btnRefresh').onclick=loadUsers;");
    h.push("document.getElementById('btnCancel').onclick=function(){document.getElementById('modal').classList.remove('on')};");
    h.push("document.getElementById('btnClose').onclick=function(){document.getElementById('dm').classList.remove('on')};");
    h.push("document.getElementById('btnSubmit').onclick=submitUser;");
    h.push("document.getElementById('ut').onclick=function(e){var b=e.target.closest('[data-act]');if(!b)return;if(b.dataset.act==='detail')showDetail(b.dataset.id);else if(b.dataset.act==='delete')deleteUser(b.dataset.id,b.dataset.name)};");
    h.push("document.getElementById('si').oninput=function(){var q=this.value.toLowerCase();renderTable(AU.filter(function(u){return u.name.toLowerCase().indexOf(q)>=0}))};");
    h.push("async function submitUser(){var name=document.getElementById('in1').value.trim();if(!name){toast('请输入用户名',true);return}var traffic=parseInt(document.getElementById('in2').value)||0;var days=parseInt(document.getElementById('in3').value)||0;var expiresAt=null;if(days>0){var d=new Date();d.setDate(d.getDate()+days);expiresAt=d.toISOString()}var editId=document.getElementById('eid').value;try{var url,body;if(editId){url='/mu/admin/updateuser/'+editId;body=JSON.stringify({name:name,monthly_bytes:traffic*1073741824,expires_at:expiresAt})}else{url='/mu/admin/createuser';body=JSON.stringify({name:name,monthly_gb:traffic,expires_at:expiresAt})}var r=await fetch(url,{method:'POST',headers:{'Content-Type':'application/json'},body:body});var d=await r.json();if(d.success||d.user){toast(editId?'已更新':'已创建');document.getElementById('modal').classList.remove('on');loadUsers()}else{toast(d.error||'操作失败',true)}}catch(e){toast('网络错误',true)}}");
    h.push("async function deleteUser(id,name){if(!confirm('确定删除用户 '+name+'?'))return;try{var r=await fetch('/mu/admin/deleteuser/'+id,{method:'POST'});var d=await r.json();if(d.success){toast('已删除');loadUsers()}else{toast(d.error||'删除失败',true)}}catch(e){toast('网络错误',true)}}");
    h.push("async function showDetail(id){try{var r=await fetch('/mu/admin/getuser/'+id);var d=await r.json();var u=d.data||d;var subUrl=location.origin+'/sub/'+u.token;var s='<div style="display:grid;gap:12px">';s+='<div style="display:flex;justify-content:space-between"><span style="color:#94a3b8">用户名</span><span>'+u.name+'</span></div>';s+='<div style="display:flex;justify-content:space-between"><span style="color:#94a3b8">UUID</span><span class="uc">'+u.uuid+'</span></div>';s+='<div style="display:flex;justify-content:space-between"><span style="color:#94a3b8">Token</span><span class="tc">'+u.token+'</span></div>';s+='<div style="display:flex;justify-content:space-between"><span style="color:#94a3b8">到期时间</span><span>'+fd(u.expires_at)+'</span></div>';s+='<div style="display:flex;justify-content:space-between"><span style="color:#94a3b8">月流量</span><span>'+(u.monthly_bytes>0?fb(u.monthly_bytes):'无限')+'</span></div>';s+='<div style="display:flex;justify-content:space-between"><span style="color:#94a3b8">已用流量</span><span>'+fb(u.used_bytes||0)+'</span></div>';s+='<hr style="border-color:#334155">';s+='<div><span style="color:#94a3b8">订阅链接</span><div style="margin-top:6px;display:flex;gap:8px"><input value="'+subUrl+'" readonly style="flex:1;background:#0f172a;border:1px solid #334155;border-radius:6px;padding:8px;color:#e2e8f0;font-size:12px;font-family:monospace" id="subInput"><button class="btn bsm bp" id="copyBtn">复制</button></div></div>';s+='</div>';document.getElementById('dt').textContent='用户详情 - '+u.name;document.getElementById('dc').innerHTML=s;document.getElementById('dm').classList.add('on');document.getElementById('copyBtn').onclick=function(){navigator.clipboard.writeText(subUrl).then(function(){toast('已复制')})}}catch(e){toast('网络错误',true)}}");
    h.push("loadUsers();");
    h.push("<\/script></body></html>");
    return new Response(h.join(''), { status: 200, headers: { 'Content-Type': 'text/html; charset=utf-8' } });
  }
